...
async function getDynamicChoices(basePath, subpath) {
    ...
    try {
        ...
        for (const item of items) {
            if (item.startsWith('.') || item === 'meta.json')
                continue;
    ...
    if (await fs.pathExists(metaPath)) {
        try {
            ...
            if (meta.notes?.length)
                meta.notes.forEach(n => console.log(chalk.gray(`   • ${n}`)));
            console.log('');
        }
        catch { }
    }
...
async function copyAndReplace(src, dest, data, ide) {
    const files = await fs.readdir(src);
    for (const file of files) {
        if (file === 'meta.json')
            continue;
        if (file === '.idea' && ide !== 'intellij')
            continue;
        if (file === '.vscode' && ide !== 'vscode')
            continue;
